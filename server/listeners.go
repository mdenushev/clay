...
func newListenerSet(opts *serverOpts) (*listenerSet, error) {
	...
	liSet.GRPC, err = newListener(opts.RPCPort, opts.Host)
	if err != nil {
	...
	if opts.RPCPort == opts.HTTPPort {
		...
		liSet.HTTP, err = newListener(opts.HTTPPort, opts.Host)
	}
	if err != nil {
...
func newListener(port int, host string) (net.Listener, error) {
